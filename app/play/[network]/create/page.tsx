...
export async function generateMetadata({ params }: { params: Promise<{ network: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: `Open a new Odd One room on ${label}, take the first seat yourself, keep the reveal ticket on this device, and invite other players onto the same network.`
  };
}
