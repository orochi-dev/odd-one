...
function RoomCard({ room }: { room: OddOneRoom }) { const phase = getPhase(room); return <Link href={`/play/${room.network}/room/${room.id}`} aria-label={roomCardAccessibleLabel(room)} className={`room-card room-${phase}`}>
...
function CreateBody({ network, client }: { network: Network; client: ReturnType<typeof useNetworkClient> }) {
  ...
  return <section className="create-layout" aria-labelledby="create-room-title" aria-describedby="create-room-summary"><div className="create-copy"><Link aria-label={`Back to the ${network === "celo" ? "Celo" : "Stacks"} lobby`} className="back-link" href={`/play/${network}`}>← Lobby</Link><span className="eyebrow">Start the 30-minute clock</span><h1 id="create-room-title">Put a number<br/>under the light.</h1><p id="create-room-summary">Your choice is hidden behind a commitment, and opening the room takes the first seat immediately. Save the reveal ticket on this device or export it; it is the only way to prove your original pick.</p><div className="phase-mini"><span><strong>20m</strong> hidden picks</span><span><strong>10m</strong> reveals</span><span><strong>12</strong> seats</span></div></div><div className="create-panel">
    {!ticket ? <><div className="panel-head"><span className="mono">YOUR SECRET NUMBER</span><strong>{number.toString().padStart(2,"0")}</strong></div><NumberPicker selected={number} onSelect={setNumber}/><fieldset className="visibility-field" aria-describedby={visibilityHintId}><legend>Room discovery</legend><label className={listed ? "active" : ""}><input type="radio" checked={listed} onChange={() => setListed(true)}/><Radio aria-hidden="true" focusable="false"/> Public <small>Shown in the lobby</small></label><label className={!listed ? "active" : ""}><input type="radio" checked={!listed} onChange={() => setListed(false)}/><LockKeyhole aria-hidden="true" focusable="false"/> Unlisted <small>Shared by link</small></label></fieldset><p id={visibilityHintId} className="sr-only">Public rooms appear in the lobby. Unlisted rooms still stay public onchain and are best shared by direct link.</p><div className="permanence-note"><EyeOff aria-hidden="true" focusable="false" size={20}/><p>Your wallet, commitment, reveal, and result will be public and permanent. Odd One cannot edit or delete a room.</p></div>{error && <p className="form-error" role="alert">{error}</p>}<button type="button" className="action action-lime full-action" onClick={() => void create()}>{client.connected ? "Commit and start room" : "Connect wallet to continue"}</button><TransactionNotice state={state}/></> : <TicketSheet ticket={ticket} backedUp={backedUp} setBackedUp={setBackedUp}/>}</div></section>;
}
...
function RoomBody({ network, id, client }: { network: Network; id: bigint; client: ReturnType<typeof useNetworkClient> }) {
  ...
  if (!client.repository.configured) return <SetupState network={network}/>; if (room === undefined) return <div className="empty-state" role="status" aria-live="polite" aria-atomic="true"><span className="loading-number" aria-hidden="true">?</span><h2>Reading the room…</h2></div>; if (room === null) return <div className="empty-state" role="alert"><span aria-hidden="true">?</span><h2>Room not found.</h2><Link aria-label={`Choose the ${networkLabel(network)} lobby from the missing room page`} className="action action-ghost" href={`/play/${network}`}>Choose a lobby</Link></div>;
  ...
  <div className="room-layout"><section className="arena-panel"><div className="arena-clock" role="timer" aria-label="Time remaining in this room"><Clock3 aria-hidden="true" focusable="false"/><span>{phase === "commit" || phase === "reveal" ? formatCountdown(remaining) : "00:00"}</span><small>{phase === "commit" ? "until reveals" : phase === "reveal" ? "left to reveal" : phase === "awaiting-finalization" ? "waiting to settle" : "final"}</small></div><div className={`player-ring count-${players.length}`}>{players.map((player, index) => <div className={`ring-player player-${index}`} key={player.wallet}><PlayerSignal address={player.wallet}/><span>{phase === "settled" || phase === "awaiting-finalization" ? player.number || "–" : player.revealed ? "✓" : "?"}</span></div>)}<div className="ring-center"><strong>{room.committedCount}</strong><small>of 12 seats</small></div></div><div className="player-list" role="list" aria-label="Room participants">{players.map((player) => <div role="listitem" key={player.wallet}><PlayerSignal address={player.wallet} size={30} decorative /><span className="mono">{shortAddress(player.wallet)}</span><span>{player.revealed ? "revealed" : phase === "commit" ? "committed" : "waiting"}</span></div>)}</div></section>
  <aside className="move-panel" aria-labelledby={movePanelTitleId}>{phase === "commit" && !joined && <><span className="eyebrow">Your move</span><h2 id={movePanelTitleId}>Pick in secret.</h2><NumberPicker selected={number} onSelect={setNumber}/><button type="button" className="action action-lime full-action" onClick={() => void join()}>{client.connected ? `Commit number ${number}` : "Connect wallet to pick"}</button></>}{phase === "commit" && joined && <><span className="eyebrow">Commitment locked</span><h2 id={movePanelTitleId}>Your number is under wraps.</h2><div className="sealed-number"><EyeOff aria-hidden="true" focusable="false"/><span>{ticket?.number ?? "?"}</span></div><p>Stay on this page or return when the reveal clock begins. Keep your ticket secret.</p>{ticket && <button type="button" aria-label={`Download reveal ticket for room ${id.toString()}`} className="action action-ghost full-action" onClick={() => { const link=document.createElement("a"); link.href=URL.createObjectURL(new Blob([JSON.stringify(ticket,null,2)])); link.download=ticketFilename(ticket); link.click(); }}>Download reveal ticket</button>}</>}{phase === "reveal" && joined && !revealed && <><span className="eyebrow">Reveal window open</span><h2 id={movePanelTitleId}>Prove your pick.</h2>{ticket ? <div className="reveal-ready"><strong>{ticket.number}</strong><span>ticket found</span></div> : <TicketImporter onFile={importTicket}/>}<button type="button" className="action action-orange full-action" disabled={!ticket} onClick={() => void reveal()}>Reveal number</button></>}{phase === "reveal" && (!joined || revealed) && <><span className="eyebrow">Reveal in progress</span><h2 id={movePanelTitleId}>{revealed ? "Your number is live." : "Spectator mode."}</h2><div className="sealed-number"><Radio aria-hidden="true" focusable="false"/><span>{revealed ? entry?.number : "?"}</span></div><p>Counts stay concealed in the normal interface until the timer ends.</p></>}{phase === "awaiting-finalization" && <><span className="eyebrow">The choices are in</span><h2 id={movePanelTitleId}>Turn on the board.</h2><CountBoard counts={counts}/><button type="button" className="action action-lime full-action" onClick={() => void finalize()}>Finalize result</button></>}{phase === "settled" && <><span className="eyebrow">Official result</span><h2 id={movePanelTitleId}>{room.outcome === "winner" ? `Number ${room.winningNumber} wins.` : room.outcome === "draw" ? "Nobody stood alone." : "No contest."}</h2><CountBoard counts={counts}/>{room.winner && <Link aria-label={`Open winner profile for ${shortAddress(room.winner)}`} className="winner-chip" href={`/play/${network}/profile/${room.winner}`}><Trophy aria-hidden="true" focusable="false"/><PlayerSignal address={room.winner} decorative /><span><small>Winner · +105</small>{shortAddress(room.winner)}</span></Link>}</>}{error && <p className="form-error" role="alert">{error}</p>}<TransactionNotice state={state}/></aside></div></>;
}

function TicketImporter({ onFile }: { onFile(file: File): void }) {
  const importHintId = useId();

  return <label className="ticket-import"><Upload aria-hidden="true" focusable="false"/><span>Import reveal ticket</span><span id={importHintId} className="sr-only">Choose the saved reveal ticket JSON file from when you committed your number.</span><input type="file" accept="application/json,.json" aria-label="Import reveal ticket" aria-describedby={importHintId} onChange={(event) => { const file=event.target.files?.[0]; if (file) onFile(file); }}/></label>;
}
...
function ProfileBody({ network, address, client }: { network: Network; address: string; client: ReturnType<typeof useNetworkClient> }) {
  ...
  const achievements=achievementTitles(stats); return <><section className="profile-hero"><Link aria-label={`Back to the ${network === "celo" ? "Celo" : "Stacks"} lobby from this player profile`} className="back-link" href={`/play/${network}`}>← Lobby</Link><div className="profile-identity"><PlayerSignal address={address} size={92} decorative /><div><span className="eyebrow">{networkLabel(network)} player</span><h1>{primaryTitle(stats.score)}</h1><p className="mono">{shortAddress(address,9)}</p></div></div><div className="score-orb"><strong>{stats.score.toString()}</strong><span>points</span></div></section>{error&&<p className="form-error" role="alert">{error}</p>}<section className="stat-grid"><article><strong>{stats.wins.toString()}</strong><span>Wins</span></article><article><strong>{stats.reveals.toString()}</strong><span>Reveals</span></article><article><strong>{stats.currentRevealStreak.toString()}</strong><span>Current streak</span></article><article><strong>{stats.bestRevealStreak.toString()}</strong><span>Best streak</span></article></section><section className="achievement-section" aria-labelledby="profile-achievement-title"><div><span className="eyebrow">Unlocked titles</span><h2 id="profile-achievement-title">The titles you earned.</h2></div><div className="achievement-list"><span className="unlocked">{primaryTitle(stats.score)}</span>{["Pattern Breaker","One of One","Unbroken"].map((title)=><span className={achievements.includes(title)?"unlocked":"locked"} key={title}>{title}</span>)}</div></section><section className="history-section" aria-labelledby="profile-history-title"><div className="section-line"><h2 id="profile-history-title">Recent rooms</h2><span className="mono">DIRECT CONTRACT HISTORY</span></div>{rooms.length?<div className="room-grid">{rooms.map((room)=><RoomCard room={room} key={room.id.toString()}/>)}</div>:<div className="empty-state compact"><span aria-hidden="true">?</span><h2>No rooms yet.</h2><p>Join a room or open your own to start building this history.</p></div>}</section></>;
}
