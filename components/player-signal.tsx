...
export function PlayerSignal({ address, size = 42, decorative = false }: { address: string; size?: number; decorative?: boolean }) {
  const seed = hash(address); const bars = Array.from({ length: 5 }, (_, i) => 5 + ((seed >> (i * 3)) & 15));
  return <svg className="player-signal" width={size} height={size} viewBox="0 0 42 42" role={decorative ? undefined : "img"} aria-hidden={decorative ? "true" : undefined} aria-label={decorative ? undefined : `Player signal for ${shortAddress(address)}`}>
