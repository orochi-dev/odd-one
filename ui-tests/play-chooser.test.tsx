...
describe("PlayChooser", () => {
  it("explains MiniPay auto-entry when no MiniPay provider is present", () => {
    ...
    expect(screen.getByRole("main")).toHaveAccessibleDescription(
      "Rooms, scores, earned titles, and room links stay separate on each network. If MiniPay is available on this device, Odd One opens the Celo lobby automatically. Otherwise, choose Celo or Stacks below.",
    );
    ...
    expect(
      screen.getByText(
        "If MiniPay is available on this device, Odd One opens the Celo lobby automatically. Otherwise, choose Celo or Stacks below.",
      ),
    ).toBeInTheDocument();
